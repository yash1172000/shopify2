...
customElements.define('load-more', LoadMore);
 ...
 const freeShippingMessage = document.querySelector('.shipping-bar');
 var promote_txt = freeShippingMessage.dataset.promote;
 var unlocked_txt = freeShippingMessage.dataset.unlocked;
 var threshold = freeShippingMessage.dataset.threshold;

// Function to update the free shipping message
function updateFreeShipping() {
  // Make a call to the Shopify AJAX API to get the current cart information
  fetch('/cart.js')
    .then(response => response.json())
    .then(data => {
      // Get the cart subtotal
      const subtotal = data.items_subtotal_price;
      var value_left = threshold - subtotal;
      var value_left_money = value_left;
       console.log(value_left);
      // Check if the cart qualifies for free shipping
      if (value_left > 0) {
        freeShippingMessage.innerHTML = '<p class="shipping-bar__message">' + promote_txt.replace('[value]',value_left_money) + '</p>';
      } else {
         freeShippingMessage.innerHTML = '<p class="shipping-bar-success shipping-bar__message">' + unlocked_txt + '</p>';
      }
    })
    .catch(error => {
...
// Call the updateFreeShipping function on page load
updateFreeShipping();
document.addEventListener('cart.updated', updateFreeShipping);
